...
public class AdvancedEmpDeptTests
{
    ...
    public void ShouldReturnEmployeeManagerPairs()
    {
        ...
        Assert.Contains(result, r => r.Employee == "ALLEN" && r.Manager == "BLAKE");
    }
    ...
    public void ShouldJoinEmpDeptSalgrade()
    {
        ...
        var result = from e in emps
                     ...
                     join s in grades on e.Sal equals s.Losal into salgradeJoin
                     from sg in salgradeJoin.DefaultIfEmpty()
                     where sg != null
                     select new { e.EName, d.DName, sg.Grade };
